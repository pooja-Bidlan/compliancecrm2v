...
import type { CFORecord } from "@/lib/cfo-data";
import { CFO_COLUMNS } from "@/lib/cfo-data";

interface Props { records: CFORecord[]; }
const PAGE_SIZE = 50;

export function CFOTable({ records }: Props) {
  ...
  const handleExport = useCallback(() => {
    ...
    const headers = CFO_COLUMNS.map(c => c.label);
    const rows = filtered.map(row => CFO_COLUMNS.map(c => `"${String(row[c.key] ?? "").replace(/"/g, '""')}"`).join(","));
    const csv = `${headers.join(",")}\n${rows.join("\n")}`;
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `CFOs_${filtered.length}.csv`; a.click();
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <Badge variant="outline" className="rounded-full text-xs px-3 py-1">{CFO_COLUMNS.length} columns</Badge>
          <Badge variant="outline" className="rounded-full text-xs px-3 py-1">500+ Employees</Badge>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative w-64">
            <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Search CFOs, companies..." value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} className="pl-9 h-9 rounded-lg bg-muted/50 border-transparent focus:border-primary/30 focus:bg-card transition-all" />
          </div>
              ...
              {pageData.map(r => (
                <>
                  <TableRow key={r.id} className="hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => setExpandedId(prev => prev === r.id ? null : r.id)}>
                    ...
                    <TableCell><span className="text-sm font-medium">{r.revenue}</span></TableCell>
                      ...
                      <TableCell colSpan={9} className="p-0">
                        <div className="p-5 animate-fade-in">
                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 text-sm">
                            ...
                            <DI label="Audit Firm" value={r.auditFirm} />
                            ...
                            <DI label="Finance Budget" value={r.annualBudget} />
