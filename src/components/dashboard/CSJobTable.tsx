...
import type { CSJobRecord } from "@/lib/cs-job-data";
import { CS_JOB_COLUMNS } from "@/lib/cs-job-data";

interface Props { records: CSJobRecord[]; }
const PAGE_SIZE = 50;

export function CSJobTable({ records }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter(r =>
      ...
      r.headquarters.toLowerCase().includes(q) || r.membershipNo.toLowerCase().includes(q)
    );
  ...
  const handleExport = useCallback(() => {
    ...
    const headers = CS_JOB_COLUMNS.map(c => c.label);
    const rows = filtered.map(row => CS_JOB_COLUMNS.map(c => `"${String(row[c.key] ?? "").replace(/"/g, '""')}"`).join(","));
    const csv = `${headers.join(",")}\n${rows.join("\n")}`;
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `CS_In_Service_${filtered.length}.csv`; a.click();
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <Badge variant="outline" className="rounded-full text-xs px-3 py-1">{CS_JOB_COLUMNS.length} columns</Badge>
          <Badge variant="outline" className="rounded-full text-xs px-3 py-1">500+ Employees</Badge>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative w-64">
            <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Search CS, companies..." value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} className="pl-9 h-9 rounded-lg bg-muted/50 border-transparent focus:border-primary/30 focus:bg-card transition-all" />
          </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/30 hover:bg-muted/30">
                ...
                <TableHead className="font-semibold text-xs uppercase tracking-wider text-muted-foreground">ICSI No.</TableHead>
              ...
              {pageData.map(r => (
                <>
                  <TableRow key={r.id} className="hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => setExpandedId(prev => prev === r.id ? null : r.id)}>
                    ...
                    <TableCell><p className="font-semibold text-foreground text-sm">{r.personName}</p><p className="text-xs text-muted-foreground">{r.yearsExperience} experience</p></TableCell>
                    <TableCell><Badge variant="outline" className="text-xs">{r.designation}</Badge></TableCell>
                    <TableCell><span className="text-xs font-mono bg-muted/50 rounded px-1.5 py-0.5">{r.membershipNo}</span></TableCell>
                      ...
                      <TableCell colSpan={9} className="p-0">
                        <div className="p-5 animate-fade-in">
                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 text-sm">
                            ...
                            <DI label="Qualifications" value={r.qualifications} />
                            <DI label="Specialization" value={r.specialization} />
                            <DI label="Board Meetings/Yr" value={r.boardMeetingsPerYear} />
                            <DI label="Reporting To" value={r.reportingTo} />
                            <DI label="Listing Status" value={r.listingStatus} />
                            <DI label="Annual Filings" value={r.annualFilings} />
                            <DI label="HQ" value={`${r.headquarters}, ${r.region}`} />
