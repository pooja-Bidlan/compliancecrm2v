...
import { ChevronDown, ChevronUp, Download, Search, Linkedin, Globe, Mail, Calculator, Phone, ChevronLeft, ChevronRight } from "lucide-react";
import type { ICAIPractitioner } from "@/lib/icai-data";
import { ICAI_COLUMNS } from "@/lib/icai-data";

interface ICAITableProps { practitioners: ICAIPractitioner[]; }
const PAGE_SIZE = 50;

export function ICAITable({ practitioners }: ICAITableProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return practitioners.filter(p =>
      p.caName.toLowerCase().includes(q) || p.firmName.toLowerCase().includes(q) ||
  ...
  const handleExport = useCallback(() => {
    const headers = ICAI_COLUMNS.map(c => c.label);
    const rows = filtered.map(row => ICAI_COLUMNS.map(c => `"${String(row[c.key] ?? "").replace(/"/g, '""')}"`).join(","));
    const blob = new Blob([`${headers.join(",")}\n${rows.join("\n")}`], { type: "text/csv" });
    const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `ICAI_Practitioners_${filtered.length}.csv`; a.click(); URL.revokeObjectURL(url);
  }, [filtered]);
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <Badge variant="outline" className="rounded-full text-xs px-3 py-1">{ICAI_COLUMNS.length} columns</Badge>
          <Badge variant="outline" className="rounded-full text-xs px-3 py-1">CA in Practice</Badge>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative w-64">
            <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Search CAs, firms, cities..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} className="pl-9 h-9 rounded-lg bg-muted/50 border-transparent focus:border-primary/30 focus:bg-card transition-all" />
          </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/30 hover:bg-muted/30">
                ...
                <TableHead className="font-semibold text-xs uppercase tracking-wider text-muted-foreground">CA Name</TableHead>
              ...
              {pageData.map(p => (
                <>
                  <TableRow key={p.id} className="hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => setExpandedId(prev => prev === p.id ? null : p.id)}>
                    ...
                    <TableCell><p className="font-semibold text-foreground text-sm">{p.caName}</p><p className="text-xs text-muted-foreground">M.No: {p.membershipNo}</p></TableCell>
                    <TableCell><p className="text-sm font-medium">{p.firmName}</p><p className="text-xs text-muted-foreground">FRN: {p.frnNo}</p></TableCell>
                      ...
                      <TableCell colSpan={9} className="p-0">
                        <div className="p-5 animate-fade-in">
                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 text-sm">
                            <DI label="Specialization" value={p.specialization} /><DI label="Region" value={p.region} />
                            <DI label="Clientele" value={p.clientele} /><DI label="Turnover" value={p.annualTurnover} />
                            <DI label="Audit Tool" value={p.auditToolUsed} /><DI label="LegalTech Need" value={p.legalTechNeed} />
...
function DI({ label, value }: { label: string; value: string | number }) {
  return <div className="space-y-1"><p className="text-xs text-muted-foreground flex items-center gap-1.5"><Calculator className="h-3.5 w-3.5" /> {label}</p><p className="font-medium text-foreground text-sm">{value}</p></div>;
}
